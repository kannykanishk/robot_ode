...
void cal_state_value(void){
	...
	AngVelLegY = cal_d_1st_filter(9, AngLegY, OMEGA);			// 脚の角速度

	AngLegX    = dJointGetUniversalAngle2(Joint_BL);			// 脚の角度
	AngVelLegX = cal_d_1st_filter(10, AngLegX, OMEGA);			// 脚の角速度

	// スライダー
	PosSlider = dJointGetSliderPosition(Joint_LS);			// スライダーの位置
	VelSlider = cal_d_1st_filter(11, PosSlider, OMEGA);		// スライダーの速度
