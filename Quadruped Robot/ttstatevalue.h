...
void cal_state_value(void){
	...
	// 脚の状態量
	AngLegY1    = dJointGetUniversalAngle1(Joint_BL1);		// 脚の角度
	AngLegX1    = dJointGetUniversalAngle2(Joint_BL1);		// 脚の角度
	AngVelLegY1 = cal_d_1st_filter(9, AngLegY1, OMEGA);		// 脚の角速度
	AngVelLegX1 = cal_d_1st_filter(10, AngLegX1, OMEGA);	// 脚の角速度
	
	AngLegY2    = dJointGetUniversalAngle1(Joint_BL2);		// 脚の角度
	AngLegX2    = dJointGetUniversalAngle2(Joint_BL2);		// 脚の角度
	AngVelLegY2 = cal_d_1st_filter(11, AngLegY2, OMEGA);	// 脚の角速度
	AngVelLegX2 = cal_d_1st_filter(12, AngLegX2, OMEGA);	// 脚の角速度

	AngLegY3    = dJointGetUniversalAngle1(Joint_BL3);		// 脚の角度
	AngLegX3    = dJointGetUniversalAngle2(Joint_BL3);		// 脚の角度
	AngVelLegY3 = cal_d_1st_filter(13, AngLegY3, OMEGA);	// 脚の角速度
	AngVelLegX3 = cal_d_1st_filter(14, AngLegX3, OMEGA);	// 脚の角速度

	AngLegY4    = dJointGetUniversalAngle1(Joint_BL4);		// 脚の角度
	AngLegX4    = dJointGetUniversalAngle2(Joint_BL4);		// 脚の角度
	AngVelLegY4 = cal_d_1st_filter(15, AngLegY4, OMEGA);	// 脚の角速度
	AngVelLegX4 = cal_d_1st_filter(16, AngLegX4, OMEGA);	// 脚の角速度

	// スライダー
	PosSlider1 = dJointGetSliderPosition(Joint_LS1);			// スライダーの位置
	VelSlider1 = cal_d_1st_filter(17, PosSlider1, OMEGA);		// スライダーの速度

	PosSlider2 = dJointGetSliderPosition(Joint_LS2);			// スライダーの位置
	VelSlider2 = cal_d_1st_filter(18, PosSlider2, OMEGA);		// スライダーの速度

	PosSlider3 = dJointGetSliderPosition(Joint_LS3);			// スライダーの位置
	VelSlider3 = cal_d_1st_filter(19, PosSlider3, OMEGA);		// スライダーの速度

	PosSlider4 = dJointGetSliderPosition(Joint_LS4);			// スライダーの位置
	VelSlider4 = cal_d_1st_filter(20, PosSlider4, OMEGA);		// スライダーの速度
