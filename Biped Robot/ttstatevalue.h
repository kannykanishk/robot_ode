...
void cal_state_value(void){
	...
	AngVelLeg1 = cal_d_1st_filter(3, AngLeg1, OMEGA);		// 脚の角速度

	AngLeg2    = dJointGetHingeAngle(Joint_BL2);			// 脚の角度
	AngVelLeg2 = cal_d_1st_filter(11, AngLeg2, OMEGA);		// 脚の角速度

	// スライダー
	PosSlider1 = dJointGetSliderPosition(Joint_LS1);			// スライダーの位置
	VelSlider1 = cal_d_1st_filter(4, PosSlider1, OMEGA);		// スライダーの速度
	...
	if(SKY && GROUND_PAST) { GroundTime = Time - time_tmp; LegChecker += 1; }	// 遊脚になった   支持脚時間の計算
	if(GROUND && SKY_PAST) { time_tmp   = Time;	LegChecker += 1; }				// 支持脚になった 支持脚時間の計算に利用
