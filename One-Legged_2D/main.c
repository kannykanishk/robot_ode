...
static void simLoop(int pause) {				// Simulation loop
	...
	if(SKY) {
		...
		if     ( DesContactPos/(Leg.l + MAX_SLIDER_POS) > sin( MAX_LEG_ANG+AngPitch) ) tmp_ang =  MAX_LEG_ANG+AngPitch;
		else if( DesContactPos/(Leg.l + MAX_SLIDER_POS) < sin(-MAX_LEG_ANG+AngPitch) ) tmp_ang = -MAX_LEG_ANG+AngPitch;
		else tmp_ang = asin(DesContactPos/(Leg.l + MAX_SLIDER_POS));
		DesLegAng    =  AngPitch - tmp_ang;
		TorqueLeg    = -100.0*(AngLeg  - DesLegAng) - 10.0*AngVelLeg;
	}
	...
	// Leg is in contact with the ground
		...
		// Torque on leg
		TorqueLeg = 1000.0*(AngPitch) + 300.0*AngVelY + 280.0*VelX;
	}

	dJointAddSliderForce(Joint_LS, ForceSlider);	// Enter force into slider joint
	dJointAddHingeTorque(Joint_BL, TorqueLeg);		// Enter torque into hinge joint
...
static void makeMonoBot() {										// Create robot bodies and joints
	...
	// Generate joints
	Joint_BL = dJointCreateHinge(world, 0);							// Generate hinge joint
	dJointAttach        (Joint_BL, Leg.body, Body.body);
	dJointSetHingeAnchor(Joint_BL, x0, y0, z0);
	dJointSetHingeAxis  (Joint_BL,  0,  1,  0);
	dJointSetHingeParam (Joint_BL, dParamLoStop, -MAX_LEG_ANG);
	dJointSetHingeParam (Joint_BL, dParamHiStop,  MAX_LEG_ANG);  
