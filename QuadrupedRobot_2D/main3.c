...
static void simLoop(int pause){					// シミュレーションループ
	...
	if(SKY){ 
		if(StepLeg1) {
			...
			if     ( DesContactPosX/(Leg2.l + MAX_SLIDER_POS) > sin( MAX_LEG_ANG+AngPitch) ) tmp_ang =  MAX_LEG_ANG + AngPitch;
			else if( DesContactPosX/(Leg2.l + MAX_SLIDER_POS) < sin(-MAX_LEG_ANG+AngPitch) ) tmp_ang = -MAX_LEG_ANG + AngPitch;
			else tmp_ang  =  asin(DesContactPosX/(Leg2.l + MAX_SLIDER_POS));
			...
			if     ( DesContactPosY/(Leg2.l + MAX_SLIDER_POS) > sin( MAX_LEG_ANG+AngRoll) ) tmp_ang =  MAX_LEG_ANG + AngRoll;
			else if( DesContactPosY/(Leg2.l + MAX_SLIDER_POS) < sin(-MAX_LEG_ANG+AngRoll) ) tmp_ang = -MAX_LEG_ANG + AngRoll;
			else tmp_ang  =  asin(DesContactPosY/(Leg2.l + MAX_SLIDER_POS));
	...
	// 支持脚時の制御則
		...
		if(StepLeg1) {
			// Leg 1,4 touching the ground, Leg 2,3 still retracted
			...
			TorqueLegY4   = TorqueLegY1;
			TorqueLegX4	  = TorqueLegX1;
		...
		if(StepLeg2) {
			// Leg 2,3 touching the ground, Leg 1,4 still retracted
	...
	dJointAddHingeTorque(Joint_BL1, TorqueLegX1);					// 脚にトルクを入力
	dJointAddSliderForce(Joint_LS2, ForceSlider2);
	dJointAddHingeTorque(Joint_BL2, TorqueLegX2);
	dJointAddSliderForce(Joint_LS3, ForceSlider3);
	dJointAddHingeTorque(Joint_BL3, TorqueLegX3);
	dJointAddSliderForce(Joint_LS4, ForceSlider4);
	dJointAddHingeTorque(Joint_BL4, TorqueLegX4);
...
static void makeMonoBot(){
	...
	dJointSetHingeAnchor(Joint_BL1, x0-0.10, y0-0.10, z0);
	dJointSetHingeAxis  (Joint_BL1,  1,  0,  0);
	...
	dJointSetHingeAnchor(Joint_BL2, x0+0.10, y0-0.10, z0);
	dJointSetHingeAxis  (Joint_BL2,  1,  0,  0);
	...
	dJointSetHingeAnchor(Joint_BL3, x0-0.10, y0+0.10, z0);
	dJointSetHingeAxis  (Joint_BL3,  1,  0,  0);
	...
	dJointSetHingeAnchor(Joint_BL4, x0+0.10, y0+0.10, z0);
	dJointSetHingeAxis  (Joint_BL4,  1,  0,  0);
