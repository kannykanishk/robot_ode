...
static float XYZ[3] = { 0.60, -2.00, 0.20 };	// Position of the viewpoint
static float HPR[3] = { 101 , -6.50, -1.2 };	// Direction of gaze
...
static void simLoop(int pause) {				// Simulation loop
	...
	dJointAddHingeTorque(Joint_BL1, TorqueLegY1);		// Enter torque into hinge joint 1
	dJointAddSliderForce(Joint_LS2, ForceSlider2);		// Enter force into slider joint 2
	dJointAddHingeTorque(Joint_BL2, TorqueLegY2);		// Enter torque into hinge joint 2
	dJointAddSliderForce(Joint_LS3, ForceSlider3);		// Enter force into slider joint 3
	dJointAddHingeTorque(Joint_BL3, TorqueLegY3);		// Enter torque into hinge joint 3
	dJointAddSliderForce(Joint_LS4, ForceSlider4);		// Enter force into slider joint 4
	dJointAddHingeTorque(Joint_BL4, TorqueLegY4);		// Enter torque into hinge joint 4
...
static void makeMonoBot() {							// Create robot bodies and joints
	...
	dBodySetPosition(Leg1.body, x0-0.15, y0, z0-Leg1.l/2);
	...
	dBodySetPosition(Leg2.body, x0+0.15, y0, z0-Leg2.l/2);
	...
	dBodySetPosition(Leg3.body, x0-0.15, y0, z0-Leg3.l/2);
	...
	dBodySetPosition(Leg4.body, x0+0.15, y0, z0-Leg4.l/2);
	...
	dBodySetPosition(Slider1.body, x0-0.15, y0, z0-(Leg1.l-Slider1.l/2));
	...
	dBodySetPosition(Slider2.body, x0+0.15, y0, z0-(Leg2.l-Slider2.l/2));
	...
	dBodySetPosition(Slider3.body, x0-0.15, y0, z0-(Leg3.l-Slider3.l/2));
	...
	dBodySetPosition(Slider4.body, x0+0.15, y0, z0-(Leg4.l-Slider4.l/2));
	...
	dJointSetHingeAnchor(Joint_BL1, x0-0.15, y0, z0);				// Set anchor point
	dJointSetHingeAxis (Joint_BL1,  0,  1,  0);						// Set joint axis (y-axis)
	dJointSetHingeParam (Joint_BL1, dParamLoStop, -MAX_LEG_ANG);	// Set min and max rotation angles along both axes
	...
	dJointSetHingeAnchor(Joint_BL2, x0+0.15, y0, z0);				// Set anchor point
	dJointSetHingeAxis  (Joint_BL2,  0,  1,  0);					// Set joint axis (y-axis)
	dJointSetHingeParam (Joint_BL2, dParamLoStop, -MAX_LEG_ANG);	// Set min and max rotation angles along both axes
	dJointSetHingeParam (Joint_BL2, dParamHiStop,  MAX_LEG_ANG); 

	Joint_BL3 = dJointCreateHinge(world, 0);						// Generate hinge joint
	dJointAttach (Joint_BL3, Leg3.body, Body.body);					// Attach bodies to joint
	dJointSetHingeAnchor(Joint_BL3, x0-0.15, y0, z0);				// Set anchor point
	dJointSetHingeAxis  (Joint_BL3,  0,  1,  0);					// Set joint axis (y-axis)
	dJointSetHingeParam (Joint_BL3, dParamLoStop, -MAX_LEG_ANG);	// Set min and max rotation angles along both axes
	dJointSetHingeParam (Joint_BL3, dParamHiStop,  MAX_LEG_ANG);

	Joint_BL4 = dJointCreateHinge(world, 0);						// Generate hinge joint
	dJointAttach (Joint_BL4, Leg4.body, Body.body);					// Attach bodies to joint
	dJointSetHingeAnchor(Joint_BL4, x0+0.15, y0, z0);				// Set anchor point
	dJointSetHingeAxis  (Joint_BL4,  0,  1,  0);					// Set joint axis (y-axis)
	dJointSetHingeParam (Joint_BL4, dParamLoStop, -MAX_LEG_ANG);	// Set min and max rotation angles along both axes
	dJointSetHingeParam (Joint_BL4, dParamHiStop,  MAX_LEG_ANG); 
