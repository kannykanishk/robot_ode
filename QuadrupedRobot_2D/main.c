...
static float XYZ[3] = { 0.60, -2.0, 0.20};	// 視点の位置
static float HPR[3] = { 101 , -6.50, -1.2   };	// 視線の方向
...
static void simLoop(int pause){					// シミュレーションループ
	...
	switch(legState) {
		...
		case 1:			//Both legs are in sky, previously Legs 2,4 were in contact with the ground
			// Leg 1,4 prepare for landing
		...
		case 3:				//Both legs are in sky, previously Legs 1,4 were in contact with the ground
			...
			// Leg 1,4 retract and at opposite angles
			ForceSlider1  = SLIDER_FORCE;
			ForceSlider4  = SLIDER_FORCE;
	...
	dJointAddHingeTorque(Joint_BL1, TorqueLegY1);		// 脚にトルクを入力
	dJointAddSliderForce(Joint_LS2, ForceSlider2);
	dJointAddHingeTorque(Joint_BL2, TorqueLegY2);
	dJointAddSliderForce(Joint_LS3, ForceSlider3);
	dJointAddHingeTorque(Joint_BL3, TorqueLegY3);
	dJointAddSliderForce(Joint_LS4, ForceSlider4);
	dJointAddHingeTorque(Joint_BL4, TorqueLegY4);
...
static void nearCallback(void *data, dGeomID o1, dGeomID o2){
	...
	// スライダーと地面が接触しているかを判定
	if((isGround == 1) && ((Slider1.geom == o1) || (Slider1.geom == o2)))  State1 = 1;		// 接触した場合
	if((isGround == 0) && ((Slider1.geom == o1) || (Slider1.geom == o2)))  State1 = 0;		// 接触していない場合
	if((isGround == 1) && ((Slider2.geom == o1) || (Slider2.geom == o2)))  State2 = 1;		// 接触した場合
	if((isGround == 0) && ((Slider2.geom == o1) || (Slider2.geom == o2)))  State2 = 0;		// 接触していない場合
...
static void makeMonoBot(){
	...
	dJointSetHingeAnchor(Joint_BL1, x0-0.15, y0, z0);
	...
	dJointSetHingeAnchor(Joint_BL2, x0+0.15, y0, z0);
	...
	Joint_BL3 = dJointCreateHinge(world, 0);
	dJointAttach (Joint_BL3, Leg3.body, Body.body);
	dJointSetHingeAnchor(Joint_BL3, x0-0.15, y0, z0);
	dJointSetHingeAxis  (Joint_BL3,  0,  1,  0);
	dJointSetHingeParam (Joint_BL3, dParamLoStop, -MAX_LEG_ANG);
	dJointSetHingeParam (Joint_BL3, dParamHiStop,  MAX_LEG_ANG);

	Joint_BL4 = dJointCreateHinge(world, 0);
	dJointAttach (Joint_BL4, Leg4.body, Body.body);
	dJointSetHingeAnchor(Joint_BL4, x0+0.15, y0, z0);
	dJointSetHingeAxis  (Joint_BL4,  0,  1,  0);
	dJointSetHingeParam (Joint_BL4, dParamLoStop, -MAX_LEG_ANG);
