...
// 脚
dReal AngLegY		= 0;					// 脚の角度	
dReal AngVelLegY	= 0;					// 脚の角速度
dReal TorqueLegY	= 0;					// 脚に加えるトルク

dReal AngLegX		= 0;					// 脚の角度	
dReal AngVelLegX	= 0;					// 脚の角速度
dReal TorqueLegX	= 0;					// 脚に加えるトルク
...
// 目標値
dReal DesVelY		 = 0;					// 目標速度
dReal DesPosY		 = 0;					// 目標位置
dReal DesLegAngY	 = 0;					// 脚の目標角度
dReal DesContactPosY = 0;					// 脚の目標接地位置

dReal DesVelX		 = 0;					// 目標速度
dReal DesPosX		 = 0;					// 目標位置
dReal DesLegAngX	 = 0;					// 脚の目標角度
...
void cal_state_value(void){
	...
	// 脚の状態量
	AngLegY    = dJointGetUniversalAngle1(Joint_BL);				// 脚の角度
	AngVelLegY = cal_d_1st_filter(11, AngLegY, OMEGA);		// 脚の角速度
	AngLegX    = dJointGetUniversalAngle2(Joint_BL);				// 脚の角度
	AngVelLegX = cal_d_1st_filter(9, AngLegX, OMEGA);		// 脚の角速度

	// スライダー
	PosSlider = dJointGetSliderPosition(Joint_LS);			// スライダーの位置
	VelSlider = cal_d_1st_filter(10, PosSlider, OMEGA);		// スライダーの速度
